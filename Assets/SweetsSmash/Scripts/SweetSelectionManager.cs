...
public class CandySelectionManager : MonoBehaviour
{
    ...
    void SwapCandy(GameObject candy, Vector2 direction)
    {
        ...
        Vector3 tempPosition = targetCandy.transform.position;
        targetCandy.transform.position = candy.transform.position;
        candy.transform.position = tempPosition;
    ...
    void SelectCandy(GameObject candy)
    {
        ...
        // Optionally log the candy's name from the Sweet component
        Sweet sweetComponent = selectedCandy.GetComponent<Sweet>();
        if (sweetComponent != null)
        {
            Debug.Log($"Selected Candy: {sweetComponent.name}"); // Get the name (e.g., "RockCandy")
        }
    }
